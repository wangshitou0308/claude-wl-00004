...
_ENDPOINT_HELP = [
    ...
    {"method": "GET/POST", "path": "/api/master 与 /api/hypotheses/{name}/chronology",
     "description": "加 ?standardization_id=（可再给 standardization_version）按已采用标准化版本的冻结轮宽指数构建主年表/年表；不传则保持原原始宽度/样本均值口径"},
    {"method": "POST", "path": "/api/crossdate 与 /api/stability",
     "description": "body 可带 standardization_id(+standardization_version) 指定已采用版本，参照/目标按冻结指数计算并随作业持久化；不传时旧作业仍保留原计算依据"},
